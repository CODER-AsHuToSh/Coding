...
// int seg[100000];
// void build_seg(int in,int lo,int hi,vi &a)
// {
// if(lo==hi)
// {
// seg[in]=a[lo];
...
// }
// int m=(lo+hi)/2;
// build_seg(2*in+1,lo,m,a);
// build_seg(2*in+2,m+1,hi,a);
...
void sol()
{
    ...
    for (int i = 0; i < c; i++)
    {
        ...
        len[i + 1] = len[i] + ops[i].second - ops[i].first + 1;
    }
    ...
    while(q--)
    {
        ...
        while(pos>n)
        {
            int val=lower_bound(len.begin(),len.end(),pos)-len.begin();
            val--;
            int diff=pos-len[val];
            pos=ops[val].first+diff-1;
        }
