...
void build(int ind,int low,int high,vi &a)
{
    if(low==high)
    {
        seg[ind]=a[low];
    ...
    int ind1 = 2 * ind + 1;
    int ind2 = 2 * ind + 2;
    build(ind1, low, mid, a);
    build(ind2, mid+1, high, a);

    seg[ind]=max(seg[ind1],seg[ind2]);
    return;
}
...
int query(int ind,int low,int high,int l,int r)
{
    ...
    int v1=query(ind1,low,mid,l,r);
    int v2=query(ind2,mid+1,high,l,r);
